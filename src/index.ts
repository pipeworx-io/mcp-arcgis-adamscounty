...
/**
 * Adams County GIS — Adams County, Colorado open geospatial data (ArcGIS).
 *
 * Adams County GIS (ArcGIS org 4PNQOtAivErR7nbT): parcels, addresses, zoning & public works. search_datasets finds layers (each
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Arcgis Adamscounty');
}


const HUB = 'https://hub.arcgis.com/api/v3/datasets';
const UA = 'pipeworx-mcp-arcgis-adamscounty/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Adams County GIS open geospatial datasets (parcels, addresses, zoning & public works) by keyword. Returns each dataset\'s name, summary, record_count, owner/org, and its Feature Service `url` — pass that url to query_layer / layer_info.',
    inputSchema: {
      type: 'object',
      properties: {
        query: { type: 'string', description: 'Keyword(s), e.g. "parcels", "crime", "flood zones".' },
        org_id: { type: 'string', description: 'Optional ArcGIS orgId to override the default (Adams County GIS).' },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    case 'search_datasets': {
      ...
      const res = await pwFetch(`${HUB}?${p}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'ArcGIS Hub');
    ...
    case 'query_layer': {
      ...
      const res = await pwFetch(`${url}/query?${p}`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'ArcGIS query');
      return res.json();
    }
    case 'layer_info': {
      const url = layerUrl(args.url);
      const res = await pwFetch(`${url}?f=json`, { headers: { Accept: 'application/json', 'User-Agent': UA } });
      if (!res.ok) throw await httpError(res, 'ArcGIS layer_info');
      return res.json();
    }
    default:
